...
THISNAME='MoS2_eos_pbe'

# Note that in this case you can optimize A and C separately
...
# The values in the lists below will be used for the parameter 'celldm' in the QuantumESPRESSO input file. 
# Check on the QE documentation what is the correct unit for this parameter.


AEXP="6.0425"
CEXP="23.243631348665723"

startA=$(echo "$AEXP-0.2" | bc -l)
endA=$(echo "$AEXP+0.2" | bc -l)  
...
if [ "$OPTIMIZE" = "optA" ]; then
  ...
  echo "Error: OPTIMIZE must be  'optA' or  'optC'"
  exit 1
fi
