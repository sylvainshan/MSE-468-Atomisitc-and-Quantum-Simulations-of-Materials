...
for supercell in $list_supercell_size; do
    sim_steps=$(echo "scale=0; $sim_time / $list_time_steps" | bc)
    echo "$sim_steps"
    stride=$(echo "scale=0; $stride_time / $list_time_steps" | bc)
    echo "$stride"
    equilibration_steps=$(echo "scale=0; $equilibration_time / $list_time_steps" | bc)
    echo "Cell size ${supercell}, Temperature ${list_temperaures} K, Timestep $list_time_steps, Equilibration time ${equilibration_steps} steps, Production $sim_steps steps"
    base_name="${supercell}_${list_temperaures}_${list_time_steps}"
...
timestep ${list_time_steps}

# Set initial temperature and velocity
velocity all create ${list_temperaures} 87287 loop geom
...
fix 1 all nvt temp ${list_temperaures} ${list_temperaures} $(echo "$list_time_steps * 100.0" | bc)
...
unfix 1
fix 2 all nve
# fix 2 all nvt temp ${list_temperaures} ${list_temperaures} $(echo "$list_time_steps * 100.0" | bc)
run ${sim_steps}
EOF
